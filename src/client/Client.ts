...
export class Client extends GameShell {
    ...
    private password: string = 'player';
    ...
    private f1Functions = [
        ...
        },
        {
            ...
            'fn': (obj: Client) => {obj.onF1Pressed_killCowsFalador();}
        },
        {
    ...
    countInvById(id: number): number {
        var cnt = 0;
        let inv = Component.types[this.inventoryComponentId];
        if (!inv || !inv.invSlotObjId || !inv.invSlotObjCount) {
            this.addMessage?.(0, 'Inventory data not available', '');
            return 0;
        }
        for (let slot = 0; slot < inv.invSlotObjId.length; slot++) {
            if (id == (inv.invSlotObjId[slot] - 1)) {
                cnt = cnt + inv.invSlotObjCount[slot];
            }
        }
        return cnt;
    }
    ...
    countBankById(id: number): number {
        ...
        for (let slot = 0; slot < inv.invSlotObjId.length; slot++) {
            if (id == inv.invSlotObjId[slot]) {
                cnt++;
                // if (inv.invSlotObjCount) this.addMessage?.(0, `Found item id ${id} with count ${inv.invSlotObjCount[slot]}`, '');
            }
        }
        return cnt;
    }
        ...
        for (let slot = 0; slot < inv.invSlotObjId.length; slot++) {
            if (id == (inv.invSlotObjId[slot] - 1)) {
                ...
                if (this.menuSize > 0) {
                    for (let i = 0; i < this.menuOption.length; i++) {
                        const optiontext = this.menuOption[i];
                        if (optiontext.startsWith('Withdraw All')) {
                            this.useMenuOption(i);
                            this.menuVisible = false;
                            if (this.menuArea === 1) {
                                this.redrawSidebar = true;
    ...
    async onF1Pressed_cookCatherby(ids: number[]) {
        ...
        let bankX = 2809;
        let bankZ = 3442;
        let outsideDoorX = 2816;
        let outsideDoorZ = 3438;
        let insideDoorX = 2816;
    ...
    */
    ...
    async tryPickDoor(doorglobalX: number, doorglobalZ: number, walkedX: number, walkedZ: number) {
        ...
        if (this.menuSize > 0) {
            for (let i = 0; i < this.menuOption.length; i++) {
                const optiontext = this.menuOption[i];
                if (optiontext.startsWith('Pick Lock')) {
                    this.useMenuOption(i);
                    this.menuVisible = false;
                    if (this.menuArea === 1) {
                        this.redrawSidebar = true;
                    ...
                    this.addMessage(0, 'Attempted to pick lock.', '');
                    await sleep(600);
                }
            }
        }

        // Wait a while and then check if we moved to the correct location.
        await sleep(2400);
        let playerglobalX = (this.localPlayer?.routeTileX[0] ?? 0) + this.sceneBaseTileX;
        let playerglobalZ = (this.localPlayer?.routeTileZ[0] ?? 0) + this.sceneBaseTileZ;
        if (playerglobalX == walkedX && playerglobalZ == walkedZ) {
            return true;
        } else {
            return false;
        }
    ...
    async onF1Pressed_thieveKnightNoRandoms() {
        ...
        while (!this.stopLoop) {
            let globalX = (this.localPlayer?.routeTileX[0] ?? 0) + this.sceneBaseTileX;
            let globalZ = (this.localPlayer?.routeTileZ[0] ?? 0) + this.sceneBaseTileZ;
            if (this.manhattanDist(globalX, globalZ, bankStandX, bankStandZ) > 200) {
                console.log('Too far from intended position. Logging out.');
                this.stopLoop = true;
                await this.logout();
                return;
    ...
    cutNearestOakTree() {
        let nearestObj = this.getNearestObjectFromArray([1281], 10);
        if (!nearestObj) {
            return false;
        }
        let a = nearestObj.fullType;
    ...
    async onF1Pressed_cutAndFletchOakTree() {
        this.stopLoop = false;
        let state = 'cutting_trees';
        let logsInvId = 1521;
        let knifeInvId = 946;
        ...
        while (!this.stopLoop) {
            ...
            if (state == 'cutting_trees') {
                while (!this.invFull()) {
                    let foundtree = this.cutNearestOakTree();
                    if (!foundtree) {
                        await sleep(200);
                        continue;
                    }
    ...
    cutNearestWillowTree() {
        let nearestObj = this.getNearestObjectFromArray([1308], 10);
        if (!nearestObj) {
            return false;
        }
        let a = nearestObj.fullType;
    ...
    async onF1Pressed_cutAndFletchWillowTree() {
        this.stopLoop = false;
        let state = 'cutting_trees';
        let logsInvId = 1519;
        let knifeInvId = 946;
